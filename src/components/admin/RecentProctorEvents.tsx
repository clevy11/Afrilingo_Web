...
export function RecentProctorEvents() {
  ...
  return (
    <Card className="border-amber-200 dark:border-[hsl(220,12%,18%)]">
      <CardHeader>
        <CardTitle className="text-amber-900 dark:text-gray-100">Recent Proctor Events</CardTitle>
      </CardHeader>
      <CardContent>
        <Table>
          <TableHeader>
          ...
          <TableBody>
            ...
            {!loading && items.length === 0 && (
              <TableRow>
                <TableCell colSpan={4} className="text-amber-700 dark:text-gray-400 text-center">No recent events</TableCell>
              </TableRow>
            )}
          </TableBody>
        </Table>
